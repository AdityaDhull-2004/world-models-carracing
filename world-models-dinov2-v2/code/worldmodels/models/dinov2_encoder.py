"""
DINOv2 ViT-Small/14 at 98x98 resolution.
98x98 -> 7x7=49 patches (14*7=98, exact multiple).
Uses timm (NOT torch.hub) — no pickling errors in multiprocessing.
This is the fix that prevents the Gen 148 crash from the previous run.
"""
import os
import numpy as np
import torch
...
class DINOv2Encoder:
    ...
    def encode_frame(self, frame):
        ...
        with torch.no_grad():
            ...
            z = feats[:, 0, :]
        return (z.cpu().numpy() * self.scaling_factor).astype(np.float32)
